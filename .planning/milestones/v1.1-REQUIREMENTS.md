# Requirements Archive: v1.1 UX — Citizen-Meaningful Controls
...
- [x] **FOCUS-01**: Staffing card focus indicator uses `outline` with `outline-offset` instead of `ring` — visible outside the selected-state `ring-2 ring-blue-600` border
- [x] **FOCUS-02**: City card focus indicator uses `outline` with `outline-offset` instead of `ring` — visible outside the selected-state `ring-2 ring-blue-600` border
